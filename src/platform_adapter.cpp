#include "quickapp/android/platform_adapter.h"
...
core::EnqueueResult SurfacePort::post(
  ...
  if (!accepting_.load(std::memory_order_acquire)) {
    return core::EnqueueResult::failure(platformError("Android Surface port is closed"));
  }
  return std::visit(
...
core::render::MeasureResult MeasurePort::measure(
  ...
  if (request.platform_font_generation == 0 || request.font_token != "system-default" ||
    ...
    return failed("Android immutable font metrics rejected MeasureRequest");
  }
  const std::size_t code_points = utf8CodePoints(request.text);
  if (code_points > 65536) return failed("Android MeasureRequest text limit exceeded");

  // A1 uses immutable, deterministic sans-serif metrics on the Core thread.
  // No Android View or UI-thread measurement participates in this result.
